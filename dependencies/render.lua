...
local function applyCasing(text, casing)
...
local function applySurround(text, surround)
	if #surround >= 2 then
		return surround:sub(1, 1) .. text .. surround:sub(2, 2);
	end
	return text;
end
...
function EspObject:Render()
	...
	local casing = interface.sharedSettings.casing;
	local surround = interface.sharedSettings.surround;
	...
	if visible.healthText.Visible then
		...
		healthText.Text = round(self.health) .. EspInterface.sharedSettings.healthSuffix;
	...
	if visible.name.Visible then
		...
		name.Text = applySurround(applyCasing(self.player.DisplayName, casing), surround);
	end
	...
	if visible.distance.Visible then
		...
		distance.Text = applySurround(applyCasing(round(self.distance) .. EspInterface.sharedSettings.distanceSuffix, casing), surround);
	...
	if visible.weapon.Visible then
		...
		weapon.Text = applySurround(applyCasing(self.weapon, casing), surround);
...
function InstanceObject:Render()
	...
	local casing = self.options.casing or "NormalCase";
	local surround = self.options.surround or "  ";
	...
	if text.Visible then
		...
		text.Text = applySurround(applyCasing(options.text
			:gsub(" {name} ", instance.Name)
			:gsub(" {distance} ", round(depth))
			:gsub(" {position} ", tostring(world)), casing), surround);
	end
end
